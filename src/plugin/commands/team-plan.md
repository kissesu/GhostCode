---
description: '多模型协作规划：TDD + PBT 零决策并行实施计划'
---
...
```
ghostcode_message_send({ text: "team-plan 启动：基于约束集 <文件名>" })
```
...
**MCP 调用**：多模型分析完成后发送消息：
```
ghostcode_message_send({ text: "多模型分析完成，开始消除歧义并拆分任务" })
```
...
**MCP 调用**：计划文件写入后发送消息：
```
ghostcode_message_send({ text: "计划已生成：<文件路径>，N 个子任务，L 个 Layer" })
```
