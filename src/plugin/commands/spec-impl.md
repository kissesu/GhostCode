---
description: '按规范执行 + 遵循计划 TDD 排序 + 多模型协作 + 归档：完整实施闭环'
---
...
```
ghostcode_message_send({ text: "spec-impl 启动：基于 <计划路径>" })
```
...
**MCP 调用**：每个 Phase 的 TDD 完成后发送消息：
```
ghostcode_message_send({ text: "Phase N 完成：Red->Green->Refactor" })
```
...
**MCP 调用**：副作用审查完成后发送消息：
```
ghostcode_message_send({ text: "副作用审查完成" })
```
...
**MCP 调用**：多模型审查通过后发送消息：
```
ghostcode_message_send({ text: "审查通过：Critical=0" })
```
...
**MCP 调用**：归档完成后发送消息：
```
ghostcode_message_send({ text: "实施完成，已归档到 <路径>" })
```
