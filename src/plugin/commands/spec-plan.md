---
description: '多模型分析 + TDD + PBT 零决策可执行计划'
---
...
```
ghostcode_message_send({ text: "spec-plan 启动：基于 <约束集路径>" })
```
...
**MCP 调用**：多模型分析完成后发送消息：
```
ghostcode_message_send({ text: "分析完成，开始综合方案并生成计划" })
```
...
**MCP 调用**：计划文件写入后发送消息：
```
ghostcode_message_send({ text: "计划已生成：<路径>" })
```
